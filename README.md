...
*Caution: Do not use this script on other datasets without verifying it will work properly. In addition to relying on the order of objects and line breaks used when exporting from JOSM, this script does not deal with relations, including multipolygons, at all.*
...
### Changesets
- Each chunk will be a separate changeset.
- Change set comment will be: ```Orange County buildings and addresses see https://wiki.openstreetmap.org/imports/. . . ```
